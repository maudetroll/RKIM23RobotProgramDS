...
class LazyPRM(PRMBase):
    ...
    @IPPerfMonitor
    def _buildRoadmap(self, numNodes, kNearest,interimGoalList):
        ...
        #print("ADDED NOTES: " + str(addedNodes))
        ...
        # Find set of candidates to connect to sorted by distance
            ...
            for data in result[1]:
                ...
                if node != c_node:
                    ...
                    #if (node in interimGoalList):
                    #    print("Verbindung zu Interim NODE: " + str(node))
                    #if (c_node in interimGoalList):
                    #    print("Verbindung zu Interim CNODE: " + str(c_node))
    ...
    def planRoundPath(self, startList, interimGoalList, goalList, config):
        ...
        self._buildRoadmap(config["initialRoadmapSize"], config["kNearest"],interimGoalList)
        ...
        while maxTry < 40: 
            try:
                ...
                self._buildRoadmap(config["updateRoadmapSize"], config["kNearest"],interimGoalList)
                maxTry += 1
                continue
