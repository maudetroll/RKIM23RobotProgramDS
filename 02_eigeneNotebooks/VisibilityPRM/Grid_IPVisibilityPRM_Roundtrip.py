...
class VisPRM(PRMBase):
    ...
    def _nearestInterim(self,currentNode,checkedInterimGoalList):
        ...
        print("interim list: ",result_interim[2] )
    ...
    def _learnRoadmap(self, ntry):
        ...
        # xCoord += gridParameter            

        # if xCoord >= plotLength:
        #     yCoord += gridParameter
        #     xCoord = 0
        for x in range(0, plotHeight, gridParameter):
            for y in range(0, plotLength, gridParameter):
                q_posList.append([x,y])
        ...
        while q_posList != []:
            ...
            q_pos = random.choice(q_posList)
            q_posList.remove(q_pos)


            if not self._collisionChecker.pointInCollision(q_pos):
    ...
    def planRoundPath(self, startList, interimGoalList, goalList, config):
        ...
        for interimGoal in range(len(checkedInterimGoalList)):
            ...
            for node in result[1]:
                print("Prüfende Node: ", self.graph.nodes()[list(posList.keys())[node]]['pos'])
                ...
                if not self._collisionChecker.lineInCollision(checkedInterimGoalList[interimGoal],self.graph.nodes()[list(posList.keys())[node]]['pos']):
                    ...
                    print("kollision erkannt ", nameOfNode)
